...
Adapted from `basics-graphics-music/labs/`. Each lab is a self-contained
`lab_top.sv` that targets the canonical uni-fpga virtual-device interface
(see `peripherals/lab_top_interface.sv`).

To build a lab on a particular board:

    PYTHONPATH=. python3 synthesize.py \
        -c nexys4_ddr_default \
        --top labs/01_and_or_not_xor/lab_top.sv \
        -o build/01_and_or_not_xor

The generated `build/<lab>/top.sv`, `constraints.xdc`, and `build.tcl` are
...
When porting a lab from `basics-graphics-music`, apply these changes:
