#!/bin/bash
# Sweep multiple boards × multiple designs through Vivado elaborate.
# Results in /tmp/board_sweep.tsv  (board\tlab\tstatus\telapsed\terror).
#
# Usage:
#   bash tools/sweep_boards.sh <board1> <board2> ...        # all designs
#   DESIGNS="06_binary_counter 1_01_*" bash tools/sweep_boards.sh <board>...
...
# Use $DESIGNS env var if set, else all designs.
if [ -z "${DESIGNS:-}" ]; then
  DESIGNS=$(ls designs/ | grep -v '^README' | grep -v '\.md$')
fi
...
for cfg in $BOARDS; do
  for design in $DESIGNS; do
    [ -f "designs/$design/design_top.sv" ] || continue
    out=$OUT/${cfg}_${design}
    rm -rf "$out"
    mkdir -p "$out"
    start=$(date +%s)
    if python3 synthesize.py -c "$cfg" --top "designs/$design/design_top.sv" -o "$out" --step elaborate \
    ...
    if [ "$rc" = 0 ]; then
      printf "%s\t%s\tOK\t%ds\t\n" "$cfg" "$design" "$elapsed" >> "$RESULTS"
    elif [ "$rc" = 2 ]; then
      # synthesize.py returns 2 when the design's // requires: block doesn't
