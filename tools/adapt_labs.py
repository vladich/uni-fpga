...
For each lab:
  ...
  2. Replace its module header (parameters + ports) with the uni-fpga
     canonical signature (matches peripherals/lab_top_interface.sv).
  ...
  4. Write to labs/<section>_<lab>/lab_top.sv.

Run with:
    python -m tools.adapt_labs           # adapt all
    python -m tools.adapt_labs --dry-run # report only
...
# Specific labs to skip — typically aggregator presentations that reference
# code in sibling directories (impractical to mechanically adapt).
SKIP_LABS = {"2024_08_31_meetup_in_la"}
...
SLOW_CLK_INSERT = """
    // ---- slow_clk derivation (auto-inserted by adapt_labs.py) -------------
...
def _strip_includes(text):
    # Strip per-board config / lab-specific includes (handled by codegen).
    text = re.sub(
...
def _strip_param_size_casts(text):
    ...
    parameter's *default* value rather than the override. When a lab uses
...
def _split_module(text):
    ...
    Walks the module declaration as: `module lab_top` [optional `# ( params )`]
    ...
    `$clog2 ( screen_width )`.
    """
    m = re.search(r"^module\s+lab_top\b", text, flags=re.MULTILINE)
    if not m:
        return None
...
def _infer_requires(body, has_slow_clk):
    """Scan the lab body and return capability requirements implied by access
    patterns. Conservative: only emit a requirement when the lab clearly
    *uses* the capability (indexed access or width-arithmetic), not when it
    ...
    # before pattern matching, so a lab that only writes `assign sound = '0`
    ...
    # 7-seg usage without an explicit indexed access — the lab uses `w_digit`
    ...
    # Screen: emit when the lab clearly paints pixels — indexed RGB access,
    ...
    # N+1 wires; labs using `gpio[N +: K]` need N+K.
    gpio_max_idx = -1
    ...
    # labs commonly tie these off with constants, which would yield false
...
# Per-lab body patches applied AFTER the standard substitutions, by lab name.
...
# Per-lab requirements that the body-scanning heuristic misses (e.g. when
...
_LAB_EXTRA_REQUIRES = {
    ...
    # Quartus rejects gpio[-1] / gpio[-2] when w_gpio=0; gate the lab
    # behind the actual GPIO requirement.
    "3_12_acoustic_locator":                            ["gpio >= 2"],
    # These labs use w_digit as a depth parameter, indexing arrays as
...
def _apply_lab_patches(lab_name, body):
    for pat, rep in _LAB_BODY_PATCHES.get(lab_name, []):
...
def _section_and_lab(path):
    """Use the directory containing lab_top.sv as the lab name. Some sections
    nest labs in subgroups (4_microarchitecture/4_1_pipelines_1/<lab>/), so
    we always pick the leaf directory."""
    rel = os.path.relpath(path, BGM_LABS)
...
def adapt_one(src_path, dry_run=False):
    ...
    # that the original lab_top declared but our canonical signature omits.
    legacy_params = []
    ...
    for name in os.listdir(src_dir):
        if not name.endswith(".sv") or name in ("lab_top.sv", "tb.sv"):
            continue
        try:
    ...
    # Merge in any per-lab extra requirements that the heuristic missed.
    extra = _LAB_EXTRA_REQUIRES.get(lab_name)
    if extra:
        if requires_block:
    ...
    out_text = (
        "// =============================================================================\n"
        "// {lab} — auto-adapted by tools/adapt_labs.py from\n"
        "//   basics-graphics-music/labs/{section}/{lab}/lab_top.sv\n"
        "// =============================================================================\n"
        "//\n"
        "{requires}"
        "\n"
        ...
        "endmodule\n"
        "{post}"
    ).format(lab=lab_name, section=section,
             ...
             post=post)
    out_dir = os.path.join(LABS_OUT, lab_name)
    if dry_run:
        return ("ok", lab_name, "would write {}".format(out_dir))
    os.makedirs(out_dir, exist_ok=True)
    with open(os.path.join(out_dir, "lab_top.sv"), "w") as f:
        f.write(out_text)
...
def main(argv=None):
    ...
    src_paths = []
    for root, _dirs, files in os.walk(BGM_LABS):
        if "lab_top.sv" in files:
            src_paths.append(os.path.join(root, "lab_top.sv"))
    src_paths.sort()
